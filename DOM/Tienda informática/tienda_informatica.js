...
function obtenerCaracteristicas(id){
	let caracteristicas = [];
	for(var i = 0; i<PRODUCTOS.length; i++){
		if(PRODUCTOS[i].id == id){
			caracteristicas =PRODUCTOS[i].caracteristicas;
		}
	}
...
function iniciar(){
	...
	for(var i = 1; i<=3; i++){
		var categoria = obtenerCategoria(i);
		var boton = `<div class='col' onclick='mostrarProductos(${i})'><h1><span class='badge bg-info cursor-pointer'>${categoria}</span></h1></div>`;
		categorias.innerHTML += boton;
	}
}
...
function mostrarProductos(id){
	...
	var productos = obtenerProductos(id);
	for(var i = 0; i<productos.length; i++){
		contenido += ` <div class="card col m-2" style="width: 18rem;">
              <img src="images/${productos[i].imagen}" class="card-img-top" alt="">
              <div class="card-body">
                <h5 class="card-title">${productos[i].nombre}</h5>
                <p class="card-text">${productos[i].descripcion}</p>
                <a onclick="mostrarCaracteristicas(${productos[i].id})" href="#" class="btn btn-primary">Características</a>
              </div>
            </div>`;
	}
...
function mostrarCaracteristicas(id){
	...
	var características = obtenerCaracteristicas(id);
	var contenido = '';
	const caracteristicasDiv = document.getElementById('caracteristicas');
	for(var i = 0; i<características.length; i++){
		contenido+=`<li class="list-group-item d-flex justify-content-between lh-sm"><h6 class="my-0">${características[i]}</h6></li>`;
	}

	caracteristicasDiv.innerHTML = contenido;
